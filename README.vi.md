# ztp-app v26.08.08 — hướng dẫn vận hành
...
3. **Config files**: upload `.conf`/`.txt`. File mới mặc định không vào Auto Pool. Mở Provisioning để bật Auto Pool, khai báo model/group hoặc `Allow any model`.
4. **Device mapping**: `STATIC` = file cố định; `AUTO` = resolver chọn file đã opt-in. Serial chỉ dùng khi đã xác nhận serial nằm cuối Option 60; MAC cần DHCP IP khi có reservation.
5. **Logs**: xem raw Option 60, DHCP lease và structured Nginx fetch. Chỉ một HTTP response đủ bytes mới chuyển `DELIVERED`/`DOWNLOADED`; HTTP 200 partial là lỗi cần xem lại.
6. Test một thiết bị trước, sau đó mới mở rộng batch. **Health** chỉ dùng sau khi thiết bị đã có management IP.
